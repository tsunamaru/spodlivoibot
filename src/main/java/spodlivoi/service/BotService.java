...
public class BotService extends TelegramLongPollingBot {
    ...
    private void acceptCommand(Message message){
        ...
        switch (command) {
            ...
            case "/test":
                ...
                break;
            case "/topdicks":
                sendMessage(message, getTopDicks(message));
                break;
            case "/bred":
    ...
    public String getTopDicks(Message mess) {
        StringBuilder message = new StringBuilder();
        int number = 1;
        Chats chat = chatRepository.getByChatId(mess.getChatId());
        if(chat == null)
            chat = registerChat(mess);
        List<Users> users = chat.getUsers();
        ArrayList<Dicks> dicks = new ArrayList<>();
        for (Users user : users) {
            try {
                dicks.add(user.getDick());
        ...
        for (Dicks dick : dicks) {
            ...
            message.append(dick.getUser().getUserName());
    ...
    public void rollDick(Message message) {
        try {
            ...
            Users user = userRepository.getByChatIdAndUserId(message.getChatId(), message.getFrom().getId());
            if(user == null)
                user = registerUser(message, chat);

            String mess = new String(dickService.roll(user).getBytes(),
